...
const DefaultItem = (props: DefaultItemProps) => {
    ...
    return (
        <AuthorityCheck userAuthority={userAuthority} authority={nav.authority}>
            <MenuItem  disabled={!roleMatched}  key={nav.key} eventKey={nav.key} dotIndent={indent}>
                <Link
                    to={roleMatched ? nav.path : pathname}
                    className={`flex items-center gap-2 h-full w-full ${!roleMatched ? 'cursor-not-allowed' : ''}`}
                    target={nav.isExternalLink ? '_blank' : ''}
                    onClick={() =>
                        onLinkClick?.({
                            key: nav.key,
                            title: nav.title,
                            path: roleMatched ? nav.path : pathname,
                        })
                    }
                >
                    {showIcon && <VerticalMenuIcon gutter='' icon={nav.icon} />}
